...
```{r}
methodists_region2 <- methodists %>%
  left_join(regions, by = "state")

methodists_region2 %>%
  group_by(year, region) %>%
  summarize(members_white = sum(members_white),
          members_black = sum(members_black),
          members_indian = sum(members_indian),
          members_total = sum(members_total)) %>% 
  mutate(percent_white = members_white / members_total, 
       percent_black = members_black / members_total, percent_indian = members_indian / members_total)%>%
  ggplot(aes(x = year)) +
  geom_line(aes(y = percent_black, color = "Percent Black")) +
  geom_line(aes(y = percent_white, color =  "Percent White")) +
  geom_line(aes(y = percent_indian, color = "Percent Indian")) +
  scale_colour_manual("", values = c("red", "blue", "green")) +
  labs(title= "Members Over Time", y = "Percentage of Members",
       x = "Year") +
...
methodists_region2 %>%
  ...
  summarize(members_white = sum(members_white),
          ...
          members_total = sum(members_total)) %>% 
  mutate(percent_white = members_white / members_total, 
       percent_black = members_black / members_total) %>%
  ggplot(aes(x = year, color = region)) +
  geom_line(aes(y = percent_black), linetype = "longdash") +
    geom_line(aes(y = percent_white)) +
      labs(title= "Members Over Time", subtitle = "*Dashed lines indicate percentage of black members", y = "Percentage of Members",
       x = "Year", color = "Region") +
...
judges_w_appoint <- judges_people %>%
  left_join(judges_appointments, by = "judge_id")

judges_w_appoint %>% 
    select(name_first, name_middle, name_last, race, court_name) %>%
  filter(court_name == "Supreme Court of the United States") %>%
...
```{r}
judges_w_appoint %>% 
    select(name_first, name_last, court_name, judge_id) %>% 
...
```{r}
gathered_methodists <- methodists_by_year_race %>% 
  gather(race, members, -year, convert = TRUE, na.rm = TRUE) 
...
```{r}
gathered_methodists %>% 
  ggplot(aes(x = year, y = members, color = race)) +
  geom_line() +
  labs(title = "Membership by Race", y = "Number of Members", x = "Year") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
wide_methodists <- gathered_methodists %>%
  spread(year, members, convert = TRUE)
...
```{r}
another_wide_methodists <- gathered_methodists %>%  
  spread(race, members)

another_wide_methodists
```
