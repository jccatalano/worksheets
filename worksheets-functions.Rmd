...
(@) What function is being called above? What are the two arguments to the function? What do those arguments do?
 ...
 Paste is the function. `c` and `collapse` are the arguments. The`paste` function concatenates the vector of strings (`c`) and `collapse` places a " -> " between each element. 
...
Hello is the name. Person is the argument. The body of the funcion is enclosed within the brackets {}. The output is a combination of "Hello" and the argument to the function. In the above example, the output is "Hello, Your name here". 
...
```{r}
convo_starter <- function(person, greeting = "Hello", pleasantry = "How are you today?"){
...
check_date_style <- function(date){
  ifelse(date >= 1752, "New Style", "Old Style")
}
...
```{r}
rescale2 <- function(x, upper_scale_limit){
  
  ((x-min(x)) / (max(x) -min(x))) * upper_scale_limit
  }
