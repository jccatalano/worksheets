...
```{r}
methodists %>%
  filter(members_total == members_black)
```
...
methodists %>% 
  filter(str_detect(meeting, ignore.case("mission")))
...
```{r}
methodists2 <- methodists %>% 
  mutate(percentage_white = members_white/members_total) %>%
  mutate(percentage_black = members_black/members_total)
```
...
methodists2 %>%
  ...
  filter(percentage_black != 1.00) %>%
  select(year, meeting, percentage_black)
```
...
methodists %>%
  filter(year == 1825) %>%
  select(year, conference, members_total) %>%
  group_by(conference) %>%
  top_n(1, members_total) 
  

methodists %>% filter(year == 1825)  %>%
  select(year, district, members_total) %>%
  group_by(district) %>%
  top_n(1, members_total)
...
methodists %>%
  filter(year >= 1786) %>%
  group_by(year) %>%
  summarize(avg_num_white = mean(members_white), avg_num_black = mean(members_black), avg_num_indian = mean(members_indian))
```
...
methodists %>%
  group_by(year) %>%
  summarize(avg_mem_black = mean(members_black, na.rm = TRUE)) %>%
  ggplot(aes(x = year, y = avg_mem_black)) +
  geom_line()
...
methodists %>% 
  group_by(year) %>% 
  summarize(avg_members = mean(members_total), median_members = median(members_total, na.rm=TRUE)) %>% 
ggplot(aes(x = year)) + 
  geom_line(aes(y = avg_members, color="Avg Number of Members")) +
  geom_line(aes(y = median_members, color="Median Number of Members")) +
  scale_colour_manual("", values = c("red", "green")) +
  labs(title= "Members Over Time", y = "Number of Members",
       x = "Year") +
...
methodists %>%
  group_by(conference, year) %>%
  summarize(avg_num_conf = mean(members_total)) %>%
  ggplot(aes(x = year, y = avg_num_conf, color= conference)) +
  geom_line() +
  labs( y = "Number of Members", x = "Year", title = "Number of Members by Conference", color = "Conference") +
  theme(plot.title = element_text(hjust = 0.5))
...
methodists %>%
  group_by(conference, year) %>%
  summarize(avg_num_conf = mean(members_total)) %>%
  ggplot(aes(x = year, y = avg_num_conf)) +
  geom_line() +
  labs( y = "Number of Members", x = "Year", title = "Number of Members by Conference") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
methodists %>%
  group_by(conference, year) %>%
  summarize(percentage_black = mean(members_black/members_total, na.rm = TRUE)) %>%
  ggplot(aes(x = year, y = percentage_black, color = conference)) +
  geom_line() +
  geom_point() +
  labs(y = "Percentage of Black Members", x = "Year", title = "Percentage of Black Members by Conference") +
    theme(plot.title = element_text(hjust = 0.5))
```
...
methodists %>%
  group_by(conference, year) %>%
  summarize(percentage_black = mean(members_black/members_total, na.rm = TRUE)) %>%
  ggplot(aes(x = year, y = percentage_black)) +
  geom_line() +
  geom_point() +
   labs(y = "Percentage of Black Members", x = "Year", title = "Percentage of Black Members by Conference") +
    theme(plot.title = element_text(hjust = 0.5)) +
